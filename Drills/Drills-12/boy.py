...
class SleepState:
    ...
    def draw(boy):
        if boy.dir == 1:
            boy.image.clip_composite_draw(int(boy.frame) * 100, 300, 100, 100, 3.141592 / 2 - boy.sp / 45, '', boy.save_x-15, boy.save_y, 100, 100)
            boy.image.opacify(1)
            ...
            boy.image.clip_composite_draw(int(boy.frame) * 100, 200, 100, 100, -3.141592 / 2 + boy.sp / 45, '', boy.save_x  +  15, boy.save_y , 100, 100)
            boy.image.opacify(1)
...
class GhostState:
    ...
    @staticmethod
    def do(boy):
        boy.x = 300 * math.sin(boy.angle * 3.14 / 360)
        boy.y = 300 + 300 * -math.cos(boy.angle * 3.14 / 360)
...
next_state_table = {
    ...
    GhostState : {LEFT_DOWN: GhostState, RIGHT_DOWN: GhostState, LEFT_UP: GhostState, RIGHT_UP: GhostState, SPACE: GhostState, Ghost : GhostState}
}
